...
namespace fugue {
namespace ida {
uint64_t input_file_size() { return retrieve_input_file_size(); }
...
std::vector<uint8_t> input_file_md5() {
  ...
  return std::vector<uint8_t>(static_cast<const uint8_t *>(hash),
                              static_cast<const uint8_t *>(hash + 16));
}
...
std::vector<uint8_t> input_file_sha256() {
  ...
  return std::vector<uint8_t>(static_cast<const uint8_t *>(hash),
                              static_cast<const uint8_t *>(hash + 32));
}
...
struct Architecture {
  ...
  Architecture(ea_t at = BADADDR) {
    ...
    bits = inf_is_64bit()   ? 64
           : inf_is_32bit() ? 32
#else
    bits = inf_is_64bit()           ? 64
           : inf_is_32bit_exactly() ? 32
#endif
                            : 16;
    is_be = inf_is_be();
...
std::string get_argument(const char *view) {
  const char *option =
      get_plugin_options((std::string("Fugue") + view).c_str());
