...
struct reloc_tools_t
{
  ...
  reloc_tools_t(elf_loader_t &_ldr, const dynamic_info_t *di_, elf_file_info_t *finfo_);
...
struct proc_def_t
{
  elf_loader_t &ldr;
  reader_t &reader;
  uint32 r_gotentry = 0;  // reloc with P inside GOT, usually R_xxx_JUMP_SLOT
...
struct elf_mips_t : public proc_def_t
{
  ...
  // relocations are permitted with "_gp_disp".
  // The special symbol name _gp_disp, used for relocating the calculation of gp
  // on entry to a DSO in 32-bit files, is not supported in ELF-64 or in the new
  // 32-bit ABI. Instead, these relocations should be composed with R_MIPS_GPREL
  // applied to an explicit symbol for the entry point of the subprogram. See the
  // examples below.
  struct gp_disp_reloc_t
  {
    ea_t P;           // address of R_MIPS_HI16 or R_MIPS_LO16 if orpahend
    uint32 AHL;       // another type of addend
    bool orphaned;    // orphaned R_MIPS_LO16
  };
  qvector<gp_disp_reloc_t> gp_disp_relocs;
...
private:
  ...
  void verify_got_displ(ea_t P, ea_t got_entry_ea, ea_t got_ea) const;
  ...
  void process_ahl_reloc(
        ...
        bool gnu_local_gp,
        bool is_rela,
        adiff_t A,      // only if IS_RELA
        uint32 insn,    // only if !IS_RELA
        reloc_tools_t *tools);
  void collect_gp_disp_reloc(ea_t P, uint32 insn, bool is_hi16);
  bool get_ssym_ea(ea_t *ssym_ea, uint8 ssym, ea_t P) const;
  void relocate_gp_disp();
...
private:
  ...
  static bool unpack_mem(qstring *out, const bytevec_t &buf, uint32 start, uint32 len, bool as_strz=false);
};
...
struct elf_loader_t
{
  ...
  bool belongs_to_skipped_section(
        elf_file_info_t &finfo,
        const sym_rel &sym);
  void detect_overlapping_pht(int64 offset, ea_t startaddr, ea_t endaddr) const;
  ...
  void guess_gotnode_size(reader_t &reader);
