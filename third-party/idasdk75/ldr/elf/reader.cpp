...
{
  if ( is_64() && !is_msb() && header.e_machine == EM_MIPS )
  {
...
void reader_t::set_rel_info_index(elf_rela_t *r, uint32 symidx) const
{
  ...
  else if ( !is_msb() && header.e_machine == EM_MIPS )
  {
        ...
        const char *counter_name)
{
  ...
  *entsize = section.sh_entsize == 0 ? 1 : section.sh_entsize;
  uint64 size = this->sections.get_size_in_file(section);
  if ( size == 0 )
    return;
  *count = size / *entsize;
#ifdef BUILD_LOADER
...
//----------------------------------------------------------------------------
bool elf_note_t::unpack_mem(qstring *out, const bytevec_t &buf, uint32 start, uint32 len, bool as_strz)
{
