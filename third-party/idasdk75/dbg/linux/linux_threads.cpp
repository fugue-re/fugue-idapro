...
//--------------------------------------------------------------------------
void linux_debmod_t::attach_to_thread(int tid, ea_t ea)
{
  ...
  ev.tid     = tid;
  ev.ea      = ea;  // TODO: move event below and ev.ea = get_ip()
  ev.handled = true;
...
bool linux_debmod_t::tdb_new(void)
{
  if ( ta == NULL )
  {
    ...
    {
      debdeb("DBG: use PTRACE_O_TRACEFORK to follow the fork events\n");
...
void linux_debmod_t::handle_extended_wait(bool *handled, const chk_signal_info_t &csi)
{
  ...
  if ( event == PTRACE_EVENT_CLONE )
  {
    ...
    thread_info_t &ti = add_thread(new_pid);
