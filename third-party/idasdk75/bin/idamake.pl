...
sub print_filtered_gcc_output
{
  ...
  while ( <$FP> )
  {
    ...
    if ( /^(\e\[\d+(;\d+)*m)*(compile|compiling|link|linking|asm|lib|qmake)(\e\[\d+(;\d+)*m)* /
      || /^(\e\[\d+(;\d+)*m)*(deploy|gendoxycfg|genhooks|genidaapi|genswigheader|gen_idc_bc695|inject_plfm|inject_pydoc|inject_base_hooks_flags|patch_codegen|patch_h_codegen|patch_python_codegen|swig|update_sdk|check_injections|split_hexrays_templates)(\e\[\d+(;\d+)*m)* /
