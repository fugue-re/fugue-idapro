...
//--------------------------------------------------------------------------
void f2mc_t::f2mc_set_device_name(const char *dev, int _respect_info)
{
  if ( dev != NULL )
  {
    ioh.device = dev;
...
void f2mc_t::choose_and_set_device(int flags)
{
  ...
  if ( choose_ioport_device2(&ioh.device, cfgname, &cb) )
    f2mc_set_device_name(ioh.device.c_str(), flags);
}
  ...
  switch ( msgid )
  {
    case processor_t::ev_init:
      ...
      if ( helper.supstr(&ioh.device, 0) > 0 )
        f2mc_set_device_name(ioh.device.c_str(), IORESP_NONE);
