...
public:
  ...
  void out_specreg(const ioports_t &table, uval_t n);
  void out_aux(uval_t n)
  {
    arc_t &pm = *static_cast<arc_t *>(procmod);
    out_specreg(pm.auxregs, n);
  }
};
...
//----------------------------------------------------------------------
void out_arc_t::out_specreg(const ioports_t &table, uval_t n)
{
  const ioport_t *reg = find_ioport(table, n);
  if ( reg == NULL )
  {
    op_t x;
    x.value = n;
...
bool out_arc_t::out_operand(const op_t & x)
{
  ...
  switch ( x.type )
  {
    ...
    case o_imm:
      ...
      //            SR <src>, [aux]
      // don't use aux register if op type is set
      if ( !is_defarg(F, x.n)
        && ((insn.itype == ARC_lr && x.n == 1 )
         || (insn.itype == ARC_sr && x.n == 1 )) )
        out_aux(x.value);
      else
        out_value(x, OOFS_IFSIGN | OOFW_IMM);
      break;
    ...
    case o_mem:
      {
        ...
        if ( (insn.auxpref & aux_pcload) != 0 )
        {
          ...
          // more readable...
          op_t y = {0};   //lint !e708 union initialization
