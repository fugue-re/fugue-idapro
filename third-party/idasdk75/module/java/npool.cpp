...
//\\VALID NULL ??? go twos if any... (reorder cur ind to null)
        ...
        CheckClassName(lm_need_cr, cd._name = cr._name, NULL);
...
{
  char diastr[128];
  loader_failure("Illegal size of declaration%s", mk_diag(apk, diastr));
}
...
void java_t::load_attributes(attr_parent_kind_t apk)
{
  ...
  for ( i = 0; i < j; i++ )
  {
    ...
    if ( savesize >= 0 && (savesize -= attribute_length) < 0 )
      loader_failure_bad_attr_decl_size(apk);
