...
{
  hook_event_listener(HT_IDP, &idp_listener, &ctx);
  hook_event_listener(HT_IDB, &idb_listener, &ctx);
}

//--------------------------------------------------------------------------
void graph_info_t::remove_hooks()
{
  unhook_event_listener(HT_IDP, &idp_listener);
...
ssize_t idaapi plugin_ctx_t::on_event(ssize_t code, va_list va)
{
  if ( code == view_close )
  {
    ...
    graph_info_t *gi = graph_info_t::find(*this, (graph_viewer_t *) cc);
    if ( gi != nullptr )
      graph_info_t::destroy_graph(*this, gi);
  }
  return 0;
